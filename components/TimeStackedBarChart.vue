...
type Methods = {
  ...
  pickBeforeTheLastNumber: (chartDataArray: number[][]) => number[]
...
> = {
  ...
  computed: {
    displayInfo() {
      ...
      const today = this.sum(this.pickLastNumber(this.chartData)).toLocaleString()

      if (this.dataKind === 'transition') {
        const yesterday =  this.sum(this.pickBeforeTheLastNumber(this.chartData)).toLocaleString()
        return {
          lText: today,
          sText: `${date} ${this.$t('日別値')}（${this.$t(
          '前日比'
        )}:  ${((Number(today) - Number(yesterday)) > 0) ? '+' : ''}${String(Number(today) - Number(yesterday))} ${this.unit}）`,
          unit: this.unit,
        }
      }
      return {
        lText: this.sum(this.cumulativeSum(this.chartData)).toLocaleString(),
        sText: `${date} ${this.$t('累計値')}（${this.$t(
          '前日比'
        )}:  ${(Number(today) > 0) ? '+'  : ''}${today} ${this.unit}）`,
        unit: this.unit,
      }
    },
    displayData() {
  ...
  methods: {
    ...
    pickBeforeTheLastNumber(chartDataArray: number[][]) {
      return chartDataArray.map((array) => {
