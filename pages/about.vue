<template>
  <div class="About">
    ...
    <static-card>
      <i18n path="当サイトは新型コロナウイルス感染症 (COVID-19) に関する最新情報を提供するために、{authorName}が開設したものです。">
          <template v-slot:authorName>
            <app-link
              to="https://tera-chan.com/"
              :icon-size="16"
            >
              {{ $t('寺田 一世') }}
            </app-link>
    ...
    <static-card>
      ...
      <p>
        {{
          $t(
            '当サイトに掲載されている情報の正確性については万全を期していますが、サイト運営者は利用者が当サイトの情報を用いて行う一切の行為について責任を負うものではありません。'
          )
        }}
      </p>
      <p>
    ...
    <static-card>
      ...
      <p>
        {{
          $t(
            '当サイトで公表しているデータは、誰でも自由にダウンロードが可能です。'
          )
        }}
